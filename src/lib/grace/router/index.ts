...
Staff read/search tools: contacts.search(query, status?, limit?)

Staff create tools: contacts.upsert, appointments.book, tasks.create, prayerRequests.create, pipelines.addToStage, memory.write, serviceRuns.createFromTemplate, serviceRuns.autoStaff

Staff update tools: contacts.update(contactId, firstName?, lastName?, email?, phone?, memberStatus?, notes?), tasks.update(taskId, title?, status?, priority?, dueDate?), tasks.complete(taskId), prayerRequests.update(requestId, status?, urgency?, response?), appointments.cancel(appointmentId)✅approval, pipeline.moveStage(itemId, stageId), ministries.addMember(ministryId, contactId, role?), conversations.resolve(conversationId)
